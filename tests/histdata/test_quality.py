...
def test_slerf_high_missing_rate() -> None:
    ...
    meta = _meta(timeframe="5m", rows=7276, missing_bars=missing,
                 interval_seconds=300.0, last_ts=1_700_000_000.0 + (total_expected - 1) * 300)
...
def test_bonk_low_missing_rate() -> None:
    ...
    meta = _meta(timeframe="1h", rows=4995, missing_bars=0, symbol="BONK",
...
def test_act_short_5m_coverage() -> None:
    ...
    meta = _meta(timeframe="5m", rows=10000, missing_bars=0,
                 first_ts=first, last_ts=last, interval_seconds=300.0)
...
def test_tier0_usable_good_series() -> None:
    ...
    meta = _meta(timeframe="1h", rows=5000, missing_bars=10,
...
def test_tier0_unusable_high_missing_1h() -> None:
    ...
    meta = _meta(timeframe="1h", rows=rows, missing_bars=missing,
                 first_ts=first, last_ts=last)
    report = build_report(meta)
...
def test_window_missing_rate_clean_window() -> None:
    ...
    meta = _meta(rows=100, missing_bars=5,
...
def test_window_missing_rate_with_gap_inside() -> None:
    ...
    meta = _meta(rows=100, missing_bars=1, gaps=(gap,),
                 first_ts=1_700_000_000.0, interval_seconds=3600.0)
...
def test_measured_tables_have_expected_coins() -> None:
    ...
    universe_coins = {
        "SLERF", "BOME", "MEW", "WIF", "POPCAT", "PNUT", "MOODENG", "GIGA",
        "AURA", "GOAT", "CHILLGUY", "PONKE", "ACT", "FWOG", "GME", "DADDY",
        "MICHI", "SC", "BILLY", "RETARDIO", "BODEN", "LOCKIN", "BONK", "MOTHER",
    }
...
def test_slerf_5m_is_highest_missing() -> None:
    ...
    assert MEASURED_5M_MISSING["SLERF"] == max(MEASURED_5M_MISSING.values())


def test_bonk_5m_is_lowest_missing() -> None:
    """BONK has the lowest 5m missing rate in the measured table."""
    assert MEASURED_5M_MISSING["BONK"] == min(MEASURED_5M_MISSING.values())
